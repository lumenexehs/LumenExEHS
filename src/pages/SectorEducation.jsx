...
import { ArrowRight, Wind, AlertTriangle, School, Radio, MessageSquare } from "lucide-react";
import { useLang } from "@/components/LanguageContext";
import { t } from "@/components/translations";

const icons = [Wind, AlertTriangle, School, Radio, MessageSquare];

export default function SectorEducation() {
  const { lang } = useLang();
  const tr = t[lang].sectorEducation;
    ...
    <div className="min-h-screen bg-white">
      <section className="relative bg-[#0F2A4A] pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-20 right-10 w-80 h-80 bg-emerald-400 rounded-full blur-3xl" />
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link to={createPageUrl("Services")}>
                <Button size="lg" variant="outline" className="bg-sky-700 text-white px-8 text-sm font-medium rounded-full border border-white/30 hover:bg-white/10">
